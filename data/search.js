...
async searchUniversity(search) {
    ...
    const universitiesList = await universityCollection.find({ name: { $regex: part } }).toArray();
    let temp = []
      for (let i = 0; i < universitiesList.length; i++) {
        //console.log(`i: ${i}, temp: ${temp}`);
        const uniList = {
          id: universitiesList[i]._id,
...
async getId(id) {
    ...
    if (!id)
    {
      ...
      const universityCollection = await universities()
      const university = await universityCollection.findOne({_id: new ObjectId(id)})
      if (!university)
      {
